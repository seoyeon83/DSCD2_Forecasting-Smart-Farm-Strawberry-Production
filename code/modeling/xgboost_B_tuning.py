...
df = pd.read_csv("최최종데이터_b_지난주추가.csv", encoding = "cp949")
...
def xgboost(df):
    ...
    # 독립 변수와 종속 변수 분리
    X_train = train_data.drop(columns=['flw_get'])  # 'flw_get'  열 제거
    y_train = train_data['flw_get']
    X_test = test_data.drop(columns=['flw_get'])  # 'flw_get' 열 제거
